      ...
      <div>
        ...
        {list && list.map((item , index)=>{
            ...
            return(
               ...
               <div key={index} className="grid grid-cols-[1fr_3fr_1fr] gap-[15px] md:grid-cols-[0.5fr_2fr_1fr_1fr_0.5fr] items-center md:gap-[10px]  p-2 border border-gray-500">

               <img src={`${url}/images/${item.image}`} className="w-20"  />
               <p className="text-sm text-gray-500">{item.name}</p>
               <p className="text-sm text-gray-500">{item.category}</p>
               <p className="text-sm text-gray-500">${item.price}</p>
               <p className=" text-red-500 cursor-pointer hover:text-red-600 text-xl">X</p>
